...
#define kNGDefaultItemHighlightColor    [UIColor colorWithWhite:1.f alpha:0.0f]
...
//        [self createGradient];
        ...
        self.backgroundColor = [UIColor colorWithRed:114/255.0f green:185/255.0f blue:127/255.0f alpha:1];
    }
...
- (void)layoutSubviews {
    ...
    for (NGTabBarItem *item in self.items) {
        ...
        frame.origin.y = currentFrameTop+20;//左侧tabbar按钮距离顶部的高度
        frame.origin.x = currentFrameLeft;
        item.frame = frame;
        
        // move to next item position
        if (NGTabBarIsVertical(self.position)) {
            currentFrameTop += frame.size.height+50;//左侧tabbar按钮之间的间距
