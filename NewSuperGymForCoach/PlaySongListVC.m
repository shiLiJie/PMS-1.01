...
-(void) addTestData{
    ...
    for (NSString *str in array) {
        //排除
        if ([str isEqualToString:kZhiliaoDS_Store]) {
            continue;
        }
        ...
        if (tmpArr.count == 1) {//首位
            if (nil == topStr) {
                ...
                NSArray *tmpArr111 = [str componentsSeparatedByString:Dian];
            ...
            if (nil == topTwoStr) {
                ...
                if ([topTwoStr isEqualToString:kZhiliaoDS_Store]) {
                    topTwoStr = nil;
                    continue;
                }
            ...
            if (topThreeStr == nil) {
                ...
                if ([topThreeStr isEqualToString:kZhiliaoDS_Store]) {
                    topThreeStr = nil;
                    continue;
                }
            }
...
- (IBAction)goBack:(id)sender {
    [UIView animateWithDuration:0.35 animations:^{
        ...
        self.view.frame = CGRectMake(200, self.view.frame.size.height+80, self.view.frame.size.width, self.view.frame.size.height);
        self.view.alpha = 0.2;
    }];
}
- (void)disasss{
    [UIView animateWithDuration:0.35 animations:^{
        ...
        self.view.frame = CGRectMake(200, self.view.frame.size.height, self.view.frame.size.width, self.view.frame.size.height);
        self.view.alpha = 0.2;
    }];
}
...
//
    if(node.type == 0){//类型为0的cell
        ...
        if(cell == nil){
            cell = [[[NSBundle mainBundle] loadNibNamed:Cell0 owner:self options:nil] lastObject];
        }
        cell.opaque = NO;
...
//        
//        cell.textLabel.backgroundColor = [UIColor clearColor];
//        [cell setBackgroundColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:0.7]];
        cell.node = node;
    ...
    if(node.type == 2){
        ...
        if (!_upIsOk) {
            if ([Tool getAutoPlaying]) {
                ...
                ToastViewMessage(ChangeShoudong);
...
//            TreatViewController *vc = [TreatViewController shareVC];
            
//            if (vc.play ) {
//                [vc.logoBg.layer removeAnimationForKey:@"1234"];
//                [vc addAnma];
//            }else{
//                vc.play = YES;
//                CFTimeInterval pausedTime = [vc.logoBg.layer timeOffset];
//                vc.logoBg.layer.speed = 1.0;
//                vc.logoBg.layer.timeOffset = 0.0;
//                vc.logoBg.layer.beginTime = 0.0;
//                CFTimeInterval timeSincePause = [vc.logoBg.layer convertTime:CACurrentMediaTime() fromLayer:nil] - pausedTime;
//                vc.logoBg.layer.beginTime = timeSincePause;}
            }
...
- (void)getTwoMenuArraylist:(NSString *)topName{
    ...
    for (CLTreeViewNode *node in _dataArray) {
        ...
        NSArray *tmp1 = [str componentsSeparatedByString:Dian];
