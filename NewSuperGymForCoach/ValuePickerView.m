...
- (void)initSubViews
{
    ...
    self.controllerToolBar.backgroundColor = RGBA(249, 249, 249, 1);
    ...
    [self.finishBtn setTitleColor:RGBA(251, 166, 84, 1) forState:UIControlStateNormal];
    self.finishBtn.titleLabel.font = [UIFont boldSystemFontOfSize:18];
    ...
    [self.cancelBtn setTitleColor:RGBA(165, 164, 160, 1) forState:UIControlStateNormal];
...
{
    //底层视图
    self.bottomView.frame = CGRectMake(10, 260, 320, self.pickerHeight);
    
    //选择器
    self.statePicker.frame = CGRectMake(0, 40, 320, self.pickerHeight - 40);
    
    //控制栏
    self.controllerToolBar.frame = CGRectMake(0, 0, 320, 40);
    
    //完成按钮
    self.finishBtn.frame = CGRectMake(320 - 70, 5, 70, 30);
    
    //取消按钮
    self.cancelBtn.frame = CGRectMake(0, 5, 70, 30);
    
    //标题
    self.titleLabel.frame = CGRectMake(70, 5, 320 - 140, 30);
}
...
- (void)show
{
    ...
    self.bottomView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
